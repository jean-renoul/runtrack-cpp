...
pingouin::pingouin(double vitesseDeNage, double vitesseDeMarche, double vitesseDeGlisse, std::string nom) 
    : aquatique(vitesseDeNage), terrestre(vitesseDeMarche), vitesseDeGlisse(vitesseDeGlisse), nom(nom), vitesseParcours(0), niveauxDeCompetence(), amis(), aventures() {
...
void pingouin::afficherNiveauxDeCompetence() {
    if (niveauxDeCompetence.empty()) {
        std::cout << "Pas de competences pour : " << this->nom << std::endl;
    }
    else {
        std::cout << this->nom << " a les competences suivantes : " << std::endl;
        for (const auto& competence : niveauxDeCompetence) {
...
void pingouin::ajouterAventure(std::string date, std::string description) {
    aventures[date] = description;
}

void pingouin::retirerAventure(std::string date) {
    aventures.erase(date);
}

void pingouin::afficherAventures() {
    if (aventures.empty()) {
        std::cout << "Pas d'aventures pour : " << this->nom << std::endl;
    }
    else {
        std::cout << this->nom << " a vecu les aventures suivantes : " << std::endl;
        for (const auto& aventure : aventures) {
            std::cout << "- " << aventure.first << " : " << aventure.second << std::endl;
        }
    }
}
